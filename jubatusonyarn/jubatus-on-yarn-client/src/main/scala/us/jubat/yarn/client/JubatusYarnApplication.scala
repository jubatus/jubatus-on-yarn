...
case class JubatusClusterConfiguration(learningMachineName: String, learningMachineType: LearningMachineType, zookeepers: List[Location], configString: String,
    configFile: Option[Path] = None, resource: Resource, nodeCount: Int, applicationName: String, serverConfig: ServerConfig, proxyConfig: ProxyConfig, logConfig: Option[LogConfig] = None, basePath: Path = new Path("hdfs:///jubatus-on-yarn"))
