...
class YarnClientSpec extends FlatSpec with Matchers {
  ...
  "submitApplicationMaster ()" should "success recource config" in {
    ...
    try {
      ...
      resource = Resource(1, 512, 2, 256, 128, 3, 256, List("1"), List("1", "2"), 3)
      jubaClusterConfig = JubatusClusterConfiguration("model1", LearningMachineType.Classifier, List(zookeeper),
        configString, None, resource, "TestApp1", ServerConfig(), ProxyConfig(), None, basePath)
      yarnClient.submitApplicationMaster(jubaClusterConfig, manageLocation)

      // ログ(submit ApplicationMaster)を目視確認
      // command: bash [entrypoint] [jar] [class] 256 [ap] [host] [port] 3 1 512 2 256 "1" "1,2" [model] [type] [zookeeper] [config] [basepath] [stdout] [stderr]
      // memory: 128+256 = 384
      // virtualCores: 3
    } catch {
      case e: Throwable =>
  ...
  it should "success log config" in {
    ...
    try {
      ...
      // 指定なし
      var jubaClusterConfig = JubatusClusterConfiguration("model1", LearningMachineType.Classifier, List(zookeeper),
        configString, None, Resource(), "DefaultLogConfig", ServerConfig(), ProxyConfig(), None, basePath)
      yarnClient.submitApplicationMaster(jubaClusterConfig, manageLocation)
      // ログ(submit ApplicationMaster)を目視確認
      ...
      jubaClusterConfig = JubatusClusterConfiguration("model1", LearningMachineType.Classifier, List(zookeeper),
        configString, None, Resource(), "AmLogConfig", ServerConfig(), ProxyConfig(), Some(amlogConfig), basePath)
      yarnClient.submitApplicationMaster(jubaClusterConfig, manageLocation)
      // ログ(submit ApplicationMaster)を目視確認
      ...
      jubaClusterConfig = JubatusClusterConfiguration("model1", LearningMachineType.Classifier, List(zookeeper),
        configString, None, Resource(), "JpLogConfig", ServerConfig(), ProxyConfig(), Some(proxylogConfig), basePath)
      yarnClient.submitApplicationMaster(jubaClusterConfig, manageLocation)
      // ログ(submit ApplicationMaster)を目視確認
      ...
      jubaClusterConfig = JubatusClusterConfiguration("model1", LearningMachineType.Classifier, List(zookeeper),
        configString, None, Resource(), "JsLogConfig", ServerConfig(), ProxyConfig(), Some(serverlogConfig), basePath)
      yarnClient.submitApplicationMaster(jubaClusterConfig, manageLocation)
      // ログ(submit ApplicationMaster)を目視確認
      ...
      jubaClusterConfig = JubatusClusterConfiguration("model1", LearningMachineType.Classifier, List(zookeeper),
        configString, None, Resource(), "AllLogConfig", ServerConfig(), ProxyConfig(), Some(alllogConfig), basePath)
      yarnClient.submitApplicationMaster(jubaClusterConfig, manageLocation)
      // ログ(submit ApplicationMaster)を目視確認
  ...
  it should "failed log config" in {
    ...
    try {
      ...
      val jubaClusterConfig = JubatusClusterConfiguration("model1", LearningMachineType.Classifier, List(zookeeper),
        configString, None, Resource(), "JpLogConfig", ServerConfig(), ProxyConfig(), Some(jplogConfig), basePath)
      yarnClient.submitApplicationMaster(jubaClusterConfig, manageLocation)
      fail()
    } catch {
      case e: Exception =>
