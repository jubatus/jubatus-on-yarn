...
class JubatusYarnApplicationSpec extends FlatSpec with Matchers with BeforeAndAfterAll {
  ...
  it should "success log config" in {
    ...
    val jubaClusterConfig = JubatusClusterConfiguration("model1", LearningMachineType.Classifier, List(zookeeper),
      configString, None, Resource(), "LogConfigTest", ServerConfig(), ProxyConfig(), logConfig, basePath)
  ...
  it should "success non-exists log config( = default value)" in {
    val logConfig = None
    val jubaClusterConfig = JubatusClusterConfiguration("model1", LearningMachineType.Classifier, List(zookeeper),
      configString, None, Resource(), "NonLogConfigTest", ServerConfig(), ProxyConfig(), logConfig, basePath)
    val future = JubatusYarnApplication.start(jubaClusterConfig)
    Await.ready(future, Duration.Inf)
    future.value.get match {
      case Success(juba) =>
  ...
  it should "illegal jubatus_server log config(illegal scheme)" in {
    ...
    val jubaClusterConfig = JubatusClusterConfiguration("model1", LearningMachineType.Classifier, List(zookeeper),
      configString, None, Resource(), "IllegalLogConfigTest_illegalScheme", ServerConfig(), ProxyConfig(), logConfig, basePath)
    val future = JubatusYarnApplication.start(jubaClusterConfig)
    Await.ready(future, Duration.Inf)
    future.value.get match {
      case Success(juba) =>
  ...
  it should "illegal jubatus_server log config(no scheme)" in {
    ...
    val jubaClusterConfig = JubatusClusterConfiguration("model1", LearningMachineType.Classifier, List(zookeeper),
      configString, None, Resource(), "IllegalLogConfigTest_noScheme", ServerConfig(), ProxyConfig(), logConfig, basePath)
    val future = JubatusYarnApplication.start(jubaClusterConfig)
    Await.ready(future, Duration.Inf)
    future.value.get match {
      case Success(juba) =>
  ...
  it should "illegal jubatus_server log config(illegal format)" in {
    ...
    val jubaClusterConfig = JubatusClusterConfiguration("model1", LearningMachineType.Classifier, List(zookeeper),
      configString, None, Resource(), "IllegalLogConfigTest_illegalFormat", ServerConfig(), ProxyConfig(), logConfig, basePath)
    val future = JubatusYarnApplication.start(jubaClusterConfig)
    Await.ready(future, Duration.Inf)
    future.value.get match {
      case Success(juba) =>
  ...
  it should "illegal jubatus_server log config(no file)" in {
    ...
    val jubaClusterConfig = JubatusClusterConfiguration("model1", LearningMachineType.Classifier, List(zookeeper),
      configString, None, Resource(), "IllegalLogConfigTest_noFile", ServerConfig(), ProxyConfig(), logConfig, basePath)
    val future = JubatusYarnApplication.start(jubaClusterConfig)
    Await.ready(future, Duration.Inf)
    future.value.get match {
      case Success(juba) =>
  ...
  it should "success resource config" in {
    val resConfig = Resource(1, 512, 2, 256, 64, 2, 256, List("1"), List("a"), 3)
    val jubaClusterConfig = JubatusClusterConfiguration("model1", LearningMachineType.Classifier, List(zookeeper),
      configString, None, resConfig, "TestApp1", ServerConfig(), ProxyConfig(), None, basePath)
    val future = JubatusYarnApplication.start(jubaClusterConfig)
    Await.ready(future, Duration.Inf)
    future.value.get match {
      case Success(juba) =>
  ...
  it should "success server config" in {
    ...
    val jubaClusterConfig = JubatusClusterConfiguration("model1", LearningMachineType.Classifier, List(zookeeper),
      configString, None, Resource(), "TestApp1", serverConfig, ProxyConfig(), None, basePath)
    val future = JubatusYarnApplication.start(jubaClusterConfig)
    Await.ready(future, Duration.Inf)
    future.value.get match {
      case Success(juba) =>
