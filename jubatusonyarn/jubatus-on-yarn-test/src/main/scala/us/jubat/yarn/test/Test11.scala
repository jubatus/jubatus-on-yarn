    ...
    val tApplicationFuture = JubatusYarnApplication.start(
      ...
      case Success(tApplication) =>
        try {
          ...
          tApplication.saveModel(new Path("hdfs:///tmp/"), "test").getOrElse { e: Throwable => throw e }

          // Thread.sleep(1000)

          println("モデルデータを読み込みます")
          tApplication.loadModel(new Path("hdfs:///tmp/"), "test").getOrElse { e: Throwable => throw e }
        } finally {
