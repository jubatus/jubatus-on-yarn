...
object Test8 extends App {
  def testcase(func: JubatusYarnApplication => Unit): Unit = {
  ...
  testcase { tApplication =>
    println("モデルデータを保存します")
    tApplication.saveModel(new Path("hdfs:///tmp/"), "test") match {
  ...
  testcase { tApplication =>
    println("モデルデータを読み込みます")
    tApplication.loadModel(new Path("hdfs:///tmp/"), "test") match {
      case Failure(e) => e.printStackTrace()
      case _ =>
    }
  }
